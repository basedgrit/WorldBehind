...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Only auto-bind a crosshair image when one is configured for the built-in crosshair flow.
        // This avoids hijacking custom reticles that are managed by other scripts.
        if (crosshairObject == null && crosshairImage != null)
        {
    ...
    void Start()
    {
        ...
        bool hasSprintBarRefs = sprintBarBG != null && sprintBar != null;

        if(useSprintBar && hasSprintBarRefs)
        {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
            {
                ...
                if (velocityChange.x != 0 || velocityChange.z != 0)
                {
                    ...
                    if (hideBarWhenFull && useSprintBar && !unlimitedSprint && sprintBarCG != null)
                    {
            ...
            else
            {
                ...
                if (hideBarWhenFull && useSprintBar && sprintRemaining == sprintDuration && sprintBarCG != null)
                {
